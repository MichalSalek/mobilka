...
type ACCOUNT_POLICY_TYPE = {
  ...
  utils: {
    IS_USER_HAS_ACCOUNT: (user: UserNoSensitiveWithRelations | null | undefined) => boolean
...
export const ACCOUNT_POLICY: ACCOUNT_POLICY_TYPE = {
  ...
  activeAccountStates: [ AccountStatus.ACTIVE,
                         AccountStatus.EXPIRING_IN_PROGRESS ],


  utils: {

    IS_USER_HAS_ACCOUNT: (user) => {
      return user
        !== null
        && typeof user
        !== 'undefined'
        && Boolean(user.account.id)
        && Boolean(!user.account.is_deleted)
    },

    IS_USER_HAS_ACTIVE_ACCOUNT: (user) => typeof user
      !== 'undefined'
      && !!user
    ...
    NEED_TO_PAY_TO_START_STATEMENT: (user) => user?.account?.account_status
      === 'NOT_ACTIVE'
      && user.account.payment_status
      === 'UNPAID'
  }
